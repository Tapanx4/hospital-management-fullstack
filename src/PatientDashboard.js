...
// import { Calendar, Clock, User, FileText, RefreshCw, PlusCircle, Briefcase, Trash2, Edit, CheckCircle, AlertTriangle, X } from 'lucide-react'; // Added Trash2, Edit, CheckCircle, AlertTriangle, X
...
//     const [loading, setLoading] = useState(true); // Covers initial load and refresh
...
//         setLoading(true); // Use main loading state for fetch
//         setError('');
//         if (!token) {
...
//         try {
//             const response = await fetch('http://localhost:8080/api/my-appointments', {
...
//                 // Keep only 'Scheduled' or 'Cancelled' appointments to show relevant ones
//                  // Sort appointments by date, newest first
//                 const sortedAppointments = data.appointments
//                     .filter(app => app.status === 'Scheduled' || app.status === 'Cancelled')
//                     .sort((a, b) => new Date(b.date) - new Date(a.date));
//                 setAppointments(sortedAppointments);
//                 console.log("PatientDashboard: Appointments state updated:", sortedAppointments);
//             } else {
...
//             console.log("PatientDashboard: Auth check done, but no token found.");
//             setLoading(false);
//         } else {
//             console.log("PatientDashboard: Waiting for initial auth check...");
//         }
//     }, [token, authLoading]);
...
//                                 {/* Show count of all fetched appointments (Scheduled + Cancelled) */}
...
//                                 You don't have any scheduled or cancelled appointments.
//                             </p>
//                         </div>
//                     ) : (
...
//                                     style={{
//                                         background: 'white', borderRadius: '16px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)',
//                                         overflow: 'hidden', border: '1px solid #e5e7eb', opacity: appointment.status === 'Cancelled' ? 0.6 : 1
//                                     }}
//                                 >
...
//                                                     {appointment.date ? new Date(appointment.date).getFullYear() : 'N/A'}
//                                                 </div>
//                                             </div>
//                                         </div>
...
//                                                     appointment.status === 'Cancelled' ? 'bg-danger text-decoration-line-through' : // Strikethrough for cancelled
//                                                     'bg-secondary'
//                                                 }`}
//                                             >
//                                                 {appointment.status}
