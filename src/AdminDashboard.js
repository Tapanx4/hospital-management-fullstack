...
// // --- Reusable Modal Component (Restyled) ---
...
//     // FIX: Define the Icon component to be used
...
//                     <h3 className={`text-2xl font-semibold ${isSuccess ? 'text-green-700' : 'text-red-700'}`}>
...
//                     <Icon 
//                         size={32} 
//                         className={`mr-4 flex-shrink-0 ${isSuccess ? 'text-green-600' : 'text-red-600'}`} 
//                     />
//                     <p className="text-gray-700 text-base" style={{ paddingTop: '5px' }}>
//                         {text}
//                     </p>
//                 </div>
...
//     // FIX: Destructure 'token' directly from the context
...
//     // NEW: State for appointments, patients, and messages
...
//                 const res = await fetch('http://localhost:8080/api/admin/doctors', fetchOptions);
//                 if (!res.ok) {
...
//         e.preventDefault();
//         if (!token) { 
//              setModalState({ isOpen: true, type: 'error', text: 'Authentication error. Cannot perform action.' });
//              return;
//         }        
//         setLoading(true);
//         try {
...
//                 endpoint = 'http://localhost:8080/api/admin/create-doctor';
//                 delete body.role; 
//             } else {
...
//             fetchData('doctors'); // Refresh the doctors list
//         } catch (err) {
//             setModalState({ isOpen: true, type: 'error', text: err.message });
//         } finally {
//             setLoading(false);
...
//              return;
//         }
//         if (!window.confirm("Are you sure? This will delete the doctor's profile AND user account.")) {
//             return;
//         }
//          try {
//             const response = await fetch(`http://localhost:8080/api/admin/doctors/${profileId}`, {
//                 method: 'DELETE',
//                 headers: { 'Authorization': `Bearer ${token}` }
//             });
...
//          if (!token) { 
//              setModalState({ isOpen: true, type: 'error', text: 'Authentication error. Cannot perform action.' });
//              setIsEditModalOpen(false); // Close edit modal on auth error
//              return;
//         }
//         setLoading(true);
//          try {
//             const { _id, displayName, specialty, bio } = editFormState;
//             const response = await fetch(`http://localhost:8080/api/admin/doctors/${_id}`, {
//                 method: 'PUT',
//                 headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
...
//             fetchData('doctors'); // Refresh list
//         } catch (err) {
//             setIsEditModalOpen(false); // Close edit modal on error
//             setModalState({ isOpen: true, type: 'error', text: err.message });
//         } finally {
//             setLoading(false);
//         }
...
//        // Use Intl.DateTimeFormat for potentially better time formatting if available
//        let timePart = timeString || '';
//        try {
//            if (timeString) {
//                 // Attempt to format timeString, assuming it's like "HH:MM"
...
//            // Fallback to original time string if formatting fails
//            timePart = timeString || '';
//        }
...
//                         <p style={{ color: '#dbeafe', marginTop: '8px', fontSize: '18px', margin: '8px 0 0 0' }}>
...
//                             <button className={`nav-link d-flex align-items-center justify-content-center ${activeTab === 'doctors' ? 'active' : ''}`} onClick={() => handleTabChange('doctors')}>
//                                 <Briefcase size={18} className="me-2" /> Manage Doctors
//                             </button>
//                         </li>
//                         <li className="nav-item">
//                             <button className={`nav-link d-flex align-items-center justify-content-center ${activeTab === 'appointments' ? 'active' : ''}`} onClick={() => handleTabChange('appointments')}>
//                                 <Calendar size={18} className="me-2" /> Manage Appointments
//                             </button>
//                         </li>
//                         <li className="nav-item">
//                             <button className={`nav-link d-flex align-items-center justify-content-center ${activeTab === 'patients' ? 'active' : ''}`} onClick={() => handleTabChange('patients')}>
//                                 <Users size={18} className="me-2" /> Manage Patients
//                             </button>
//                         </li>
//                         <li className="nav-item">
//                             <button className={`nav-link d-flex align-items-center justify-content-center ${activeTab === 'messages' ? 'active' : ''}`} onClick={() => handleTabChange('messages')}>
//                                 <MessageSquare size={18} className="me-2" /> View Messages
//                             </button>
//                         </li>
//                     </ul>

//                     {/* --- Tab Content --- */}
//                     <div className="tab-content pt-3"> {/* Added padding top */}
...
//                                     {/* --- Create Account Form --- */}
//                                     <div className="bg-white p-4 rounded shadow h-100"> {/* Added h-100 */}
//                                         <h2 className="text-xl font-bold mb-4 border-bottom pb-3">Create New Account</h2>
//                                         <form onSubmit={handleSubmit}>
//                                             <div className="mb-3">
//                                                 <label htmlFor="role" className="form-label">Account Type</label>
//                                                 <select className="form-select" id="role" name="role" value={formState.role} onChange={handleChange} required>
...
//                                                 <>
//                                                     <hr className="my-3" /> {/* Reduced margin */}
//                                                     <h4 className="text-lg font-semibold mb-3">Doctor Profile Details</h4>
//                                                     <div className="mb-3">
//                                                         <label htmlFor="displayName" className="form-label">Display Name</label>
//                                                         <input type="text" className="form-control" id="displayName" name="displayName" value={formState.displayName} onChange={handleChange} required={formState.role === 'doctor'} />
//                                                     </div>
//                                                     <div className="mb-3">
//                                                         <label htmlFor="specialty" className="form-label">Specialty</label>
//                                                         <input type="text" className="form-control" id="specialty" name="specialty" value={formState.specialty} onChange={handleChange} required={formState.role === 'doctor'} />
//                                                     </div>
//                                                     <div className="mb-3">
//                                                         <label htmlFor="bio" className="form-label">Bio (Optional)</label>
//                                                         <textarea className="form-control" id="bio" name="bio" value={formState.bio} onChange={handleChange}></textarea>
//                                                     </div>
//                                                 </>
//                                             )}
//                                             <button type="submit" className="btn btn-success w-100 mt-3" disabled={loading}>
//                                                 {loading ? 'Creating...' : `Create ${formState.role} Account`}
//                                             </button>
//                                         </form>
//                                     </div>
//                                 </div>
...
//                                             <table className="table table-striped table-hover table-sm"> {/* Added table-sm */}
//                                                 <thead>
//                                                     <tr>
//                                                         <th>Display Name</th>
//                                                         <th>Username</th>
...
//                                                     ) : doctors.length === 0 ? (
//                                                         <tr>
//                                                             <td colSpan="4" className="text-center text-muted py-3">No doctor profiles found.</td>
//                                                         </tr>
//                                                     ) : (
//                                                         doctors.map(profile => (
//                                                             <tr key={profile._id}>
...
//                                         <tbody>
//                                             {patients.length === 0 ? (
//                                                 <tr><td colSpan="2" className="text-center text-muted py-3">No patients found.</td></tr>
//                                             ) : (
//                                                 patients.map(p => (
//                                                     <tr key={p._id}>
...
//                                 {messages.length === 0 ? (
//                                     <p className="text-center text-muted py-3">No messages found.</p>
//                                 ) : (
//                                     messages.map(msg => (
//                                         <div key={msg._id} className="border rounded mb-3 bg-light">
//                                             <div className="p-3 border-bottom d-flex justify-content-between align-items-center">
//                                                 <div>
//                                                     <strong className="me-3 fs-5">{msg.name}</strong>
//                                                     <span className="text-muted me-3">&lt;{msg.email}&gt;</span>
//                                                 </div>
//                                                 <span className="text-muted small">{formatDate(msg.createdAt)}</span>
//                                             </div>
//                                             <div className="p-3">
//                                                 <h5 className="font-semibold mb-2">{msg.subject || '(No Subject)'}</h5>
//                                                 <p className="mb-0 text-secondary">{msg.message}</p>
//                                             </div>
//                                         </div>
//                                     ))
//                                 )}
